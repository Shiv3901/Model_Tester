...
to setup
  ...
  set wei_1 set-weights [4 91 3 2] 0
  set wei_2 set-weights [2 3 4 91] 0
  set wei_3 set-weights (list w1 w2 w3 w4) 0
  set wei_4 set-weights [1 99 1 99] 0
...
to choose-new-lane ; turtle procedure
  ...
  if not empty? other-lanes [
    ...
    if (decision = 1) [
      ...
      ;show other-lanes
      ;show deci_1_lanes
      ;show lanes-for-deci-1 other-lanes
    ]
...
to-report times-1000 [values]
  report map [i -> i * 1000] values
end
...
to-report row-value [values n]
  ...
  let n-vals n-values (length values) [n]
  let n-vals-product reduce * n-vals
  let num (total) / (n-vals-product)
  report (nth-root num (length values))
...
to-report invert-values [values]
  ...
  if not empty? locations [
    ...
    foreach locations [x -> set values replace-item x values 1]
  ]
...
; thinking ends her7.55629439322956E-5e


to-report mat

  ; let m matrix:from-row-list [ [1 9 2.5 3 6] [0.111 1 0.2 0.286 0.678] [0.4 5 1 1.7 3] [ 0.333 3.5 0.588 1 1.5 ] [0.167 1.5 0.333 0.678 1] ]
  let m matrix:from-row-list [ [1 0.333 5] [3 1 7] [0.2 0.142 1] ]
  show matrix:imaginary-eigenvalues m
  show matrix:eigenvectors m
  report 1
